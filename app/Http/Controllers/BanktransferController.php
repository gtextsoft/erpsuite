...
class BanktransferController extends Controller
{
    ...
    public function invoicePayWithBank(Request $request)
    {
        ...
        elseif($request->type == 'feespayment'){
        ...
        if($invoice){
            ...
            elseif($request->type == 'feespayment'){
                return redirect()->route('pay.fees',\Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('success', __('Fees payment request send successfully').('<br> <span class="text-danger"> '.__('Your request will be approved by company and then your payment will be activated.').'</span>'));
            }
    ...
    public function invoiceBankRequestEdit($id)
    {
        ...
        if($bank_transfer_payment)
        {
            ...
            elseif($bank_transfer_payment->type == 'feespayment'){
                $feesReceipt = \Modules\LegalCaseManagement\Entities\FeesReceipt::find($bank_transfer_payment->request);
                $invoice_id = $feesReceipt->bill_number;
            }
        ...
        if($bank_transfer_payment && $bank_transfer_payment->status == 'Pending')
        {
            ...
            if($request->status == 'Approved')
            {
                ...
                elseif($bank_transfer_payment->type == 'feespayment'){
                    ...
                    $feesPayment                    = new \Modules\LegalCaseManagement\Entities\FeesReceiptPayment();
                    $feesPayment->fees_reciept_id   = $feesReceipt->id;
                    $feesPayment->date              = Date('Y-m-d');
                    $feesPayment->amount            = $bank_transfer_payment->price;
                    $feesPayment->method            = 'Bank Transfer';
                    $feesPayment->order_id          = $bank_transfer_payment->order_id;
                    $feesPayment->currency          = $bank_transfer_payment->price_currency;
                    $feesPayment->reciept           = $bank_transfer_payment->attachment;
                    $feesPayment->status            = $bank_transfer_payment->status;
                    $feesPayment->save();

                    $feesReceipt->due_amount = $feesReceipt->due_amount - $bank_transfer_payment->price;
                    if($feesReceipt->due_amount - $bank_transfer_payment->price <= 0){
                        $feesReceipt->status = 'PAID';
                        $feesReceipt->save();
                    }
                    else{
                        $feesReceipt->status = 'Partialy Paid';
                        $feesReceipt->save();
                    }

                    event(new BankTransferPaymentStatus($feesReceipt,$bank_transfer_payment->type,$feesPayment));
                }
                $bank_transfer_payment->delete();
                return redirect()->back()->with('success', __('Bank transfer request Approve successfully'));
            }
