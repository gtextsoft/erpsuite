...
//             'email' => 'required|string|email',
//             'password' => 'required|string',
//             'workspace_id' => 'nullable|integer|exists:work_spaces,id', // Make workspace_id optional
//         ]);
...
//         if ($users->count() > 1 && !$request->workspace_id) {
//             // Return workspace options if multiple users exist
//             return response()->json([
//                 'status' => 0,
//                 'message' => 'Multiple accounts found. Please select a workspace.',
//                 'workspaces' => $users->map(function ($u) {
//                     $workspace = WorkSpace::find($u->workspace_id);
//                     return [
//                         'workspace_id' => $u->workspace_id,
//                         'workspace_name' => $workspace ? $workspace->name : 'Unknown',
//                     ];
//                 })
//             ], 400);
//         }

//         // Find user by email and optional workspace_id
//         $query = User::where('email', $credentials['email']);
//         if ($request->workspace_id) {
//             $query->where('workspace_id', $request->workspace_id);
//         } else {
//             $query->orderBy('updated_at', 'desc'); // Fallback to most recent user
//         }
//         $user = $query->first();
...
//                 'workspaces' => $this->getWorkspaceArray($user->id),
//             ]
//         ], 200);
...
//         \Log::info('API Login Attempt - Email: ' . $request->email . ', Password: ' . $request->password);
...
//         $token = \JWTAuth::attempt($credentials);
//         if (!$token) {
//             \Log::error('JWTAuth::attempt failed for user: ' . $credentials['email']);
//             return response()->json([
//                 'status' => 0,
//                 'message' => 'Invalid credentials',
//             ], 401);
...
//         return response()->json(['status' => 0, 'message' => 'JWT Authentication Error'], 500);
//     } catch (\Exception $e) {
...
// //     public function changePassword(Request $request)
// // {
// //     $user = User::where('email', 'emailtest@gtextmails.com')->first();
