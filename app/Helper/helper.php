...
if (!function_exists('generateSubMenu')) {
    function generateSubMenu($menuItems, $parent = null)
    {
        ...
        foreach ($filteredItems as $item) {
            ...
            $targetAttr = !empty($item['target']) ? ' target="' . $item['target'] . '"' : '';
            $html .= '<a href="' . $href . '"' . $targetAttr . ' class="dash-link">';

            if ($item['parent'] == null) {
                // Handle empty icon gracefully
                $icon = !empty($item['icon']) ? $item['icon'] : 'circle';
                $html .= ' <span class="dash-micon"><i class="ti ti-' . $icon . '"></i></span>
...
if (!function_exists('getGeneralWorkSpace')) {
    function getGeneralWorkSpace($user_id = null)
    {
        if (!empty($user_id)) {
            $user = User::find($user_id);
        } else {
        ...
        if ($user) {
            $generalWorkspace = WorkSpace::where('name', 'general')
                ->first();

            return $generalWorkspace ? $generalWorkspace->id : null;
        }

        return null; // Return null if user or workspace not found
    }
}
    ...
    function check_file($path)
    {
        if (!empty($path)) {
            ...
            if ($storage_setting == null || $storage_setting == 'local') {
                ...
                $possiblePaths = [
                    public_path(str_replace('uploads/', '', $path)),
                    base_path($path),
                ...
                // Cloud storage handling with proper error checking
                try {
                    if ($storage_setting == 's3' && isset($storage_settings['s3_key'])) {
                        config([
                            'filesystems.disks.s3.key' => $storage_settings['s3_key'],
                            'filesystems.disks.s3.secret' => $storage_settings['s3_secret'],
                            'filesystems.disks.s3.region' => $storage_settings['s3_region'],
                            'filesystems.disks.s3.bucket' => $storage_settings['s3_bucket'],
                            'filesystems.disks.s3.url' => $storage_settings['s3_url'] ?? null,
                            'filesystems.disks.s3.endpoint' => $storage_settings['s3_endpoint'] ?? null,
                        ]);
                    } elseif ($storage_setting == 'wasabi' && isset($storage_settings['wasabi_key'])) {
                        config([
                            'filesystems.disks.wasabi.key' => $storage_settings['wasabi_key'],
                            'filesystems.disks.wasabi.secret' => $storage_settings['wasabi_secret'],
                            'filesystems.disks.wasabi.region' => $storage_settings['wasabi_region'],
                            'filesystems.disks.wasabi.bucket' => $storage_settings['wasabi_bucket'],
                            'filesystems.disks.wasabi.root' => $storage_settings['wasabi_root'] ?? null,
