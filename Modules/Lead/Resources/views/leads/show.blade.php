...
@section('page-action')
    <div>
       ...
       @permission('lead to deal convert')
            @if(!empty($deal))
                    <a href="@permission('deal show') @if($deal->is_active) {{route('deals.show',$deal->id)}} @else # @endif @else # @endpermission" class="btn btn-sm btn-primary btn-icon " data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Already Converted To Deal')}}"><i class="ti ti-exchange text-white"></i></a>
            @else
                                        ...
                                        <div class="col-md-4 col-sm-4 mt-4">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <div class="ms-2">
                                                    ...
                                                    <h5 class="mb-0 {{ $lead->follow_up_date != null && $lead->follow_up_date < date('Y-m-d') ? 'text-danger' : 'text-primary' }}">{{ isset($lead->follow_up_date) && !empty($lead->follow_up_date) ? company_date_formate($lead->follow_up_date) : '-'}}</h5>
                                                </div>
                                            </div>
                                        </div>
                                        @if(!empty($customFields) && count($lead->customField) > 0)
                                            @foreach($customFields as $key => $field)
                                                <div class="col-md-4 col-sm-4 mt-4">
                                                    <div class="d-flex align-items-start">
                                                        @if ($key % 3 === 2)
                                                            <div class="theme-avtar bg-primary {{ $field->type == 'textarea' && !empty($lead->customField[$field->id]) ? 'mt-2' : '' }}">
                                                                <i class="ti ti-circle-plus"></i>
                                                            </div>
                                                            <div class="ms-2">
                                                                <strong>{{Str::ucfirst($field->name)}}</strong><br>
                                                                @if ($field->type == 'attachment')
                                                                    <a href="{{ !empty($lead->customField[$field->id]) ? get_file($lead->customField[$field->id]) : get_file('Modules/Lead/Resources/assets/upload/default-img.png') }}"
                                                                        target=_blank
                                                                        class="btn btn-md p-0"
                                                                        data-bs-toggle="tooltip" title="{{ __('Preview') }}">
                                                                        <h5 class="text-primary">{{ __('Preview Attachment') }}
                                                                ...
                                                                @else
                                                                    @if ($field->type == 'textarea')
                                                                        <div class="description-container" data-key="{{ $key }}">
                                                                            <h5 class="text-primary shortDescription" id="shortDescription{{$key}}">
                                                                                {{ \Illuminate\Support\Str::limit(!empty($lead->customField[$field->id]) ? $lead->customField[$field->id] : '-', 40) }}
                                                                            </h5>
                                                                            <h5 class="text-primary fullDescription" style="display: none;">
                                                                                {{ !empty($lead->customField[$field->id]) ? $lead->customField[$field->id] : '-' }}
                                                                            </h5>
                                                                        </div>
                                                                    @else
                                                                        <h5 class="text-primary">{{ !empty($lead->customField[$field->id]) ? $lead->customField[$field->id] : '-' }}</h5>
                                                                    @endif
                                                                @endif
                                                            </div>
                                                        @elseif($key % 3 === 0)
                                                            <div class="theme-avtar bg-warning {{ $field->type == 'textarea' && !empty($lead->customField[$field->id]) ? 'mt-2' : '' }}">
                                                                <i class="ti ti-circle-plus"></i>
                                                            </div>
                                                            <div class="ms-2">
                                                                <strong>{{Str::ucfirst($field->name)}}</strong><br>
                                                                ...
                                                                @else
                                                                    ...
                                                                    @else
                                                                        <h5 class="text-warning">{{ !empty($lead->customField[$field->id]) ? $lead->customField[$field->id] : '-' }}</h5>
                                                                    @endif
                                                                @endif
                                                            </div>
                                                        @elseif($key % 3 === 1)
                                                            <div class="theme-avtar bg-info {{ $field->type == 'textarea' && !empty($lead->customField[$field->id]) ? 'mt-2' : '' }}">
                                                                <i class="ti ti-circle-plus"></i>
                                                            </div>
                                                            <div class="ms-2">
                                                                <strong>{{Str::ucfirst($field->name)}}</strong><br>
                                                                ...
                                                                @else
                                                                    ...
                                                                    @else
                                                                        <h5 class="text-info">{{ !empty($lead->customField[$field->id]) ? $lead->customField[$field->id] : '-' }}</h5>
                                                                    @endif
                                                                @endif
                                                            </div>
                                                        @endif
                                                ...
                                                <table class="table align-items-center mb-0">
                                                    <tbody class="list">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <span class="number-id">{{$product->name}} </span> (<span class="text-muted">{{ currency_format_with_sym($product->sale_price) }}</span>)
                                                            </td>
                                                            @permission('lead edit')
                                                                <td>
                                                                    <div class="action-btn bg-danger ms-2">
                            ...
                            <div class="col-6">
                                <div class="card">
                                    ...
                                    <div class="card-body" style="height:330px;">
                                        <textarea name="notes" class="form-control summernote {{ !empty($errors->first('description')) ? 'is-invalid' : '' }}" required id="lead-summernote">{!! $lead->notes !!}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
