...
use Modules\Lead\Entities\SlackMessage;
...
class LeadController extends Controller
{
    ...
    public function dashboard()
    {
        if (Auth::user()->isAbleTo('crm dashboard manage')) {
            ...
            $deals = Deal::where('created_by', '=', $creatorId)->where('workspace_id', '=', $getActiveWorkSpace)->take(5)->with('stage')->get();
            $user = Auth::user()->name;
    ...
    public function store(Request $request)
    {
        ...
        if ($usr->isAbleTo('lead create')) {
            ...
            if (empty($stage)) {
                ...
                $sanitizedWorkspace = Str::upper(Str::slug($workspaceName, '_')); // Convert to uppercase and replace spaces with '_'
                $uniqueId = time(); // Unique timestamp (or use Str::random(6) for a random string)
                ...
                $lead->short_description = $request->short_description;
                $lead->save();
                ...
                if (!empty($company_settings['Lead Assigned']) && $company_settings['Lead Assigned']  == true) {
                    $lArr    = [
                        'lead_name' => $lead->name,
    ...
    public function lead_list()
    {
        ...
        if ($usr->isAbleTo('lead manage')) {
            ...
            $leads     = Lead::select('leads.*')->join('user_leads', 'user_leads.lead_id', '=', 'leads.id')
                ...
                ->orderBy('leads.order')->with('stage')->get();
            
            //  $leads     = Lead::select('leads.*')
            //     ->where('leads.user_id', '=', $usr->id)
            //     ->where('leads.pipeline_id', '=', $pipeline->id)
            //     ->where('leads.workspace_id', $getActiveWorkSpace)
            //     ->orderBy('leads.order')->with('stage')->get();
    ...
    public function order(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('lead move')) {
            ...
            if ($lead->stage_id != $post['stage_id']) {
                ...
                $company_settings = getCompanyAllSetting();
                if (!empty($company_settings['Lead Moved']) && $company_settings['Lead Moved']  == true) {
    ...
    public function convertToDeal($id, Request $request)
    {
        ...
        $company_settings = getCompanyAllSetting();
        if (!empty($company_settings['Deal Assigned']) && $company_settings['Deal Assigned']  == true) {
    ...
    public function emailStore($id, Request $request)
    {
        if (Auth::user()->isAbleTo('lead email create')) {
            ...
            if ($lead->created_by == creatorId() && $lead->workspace_id == getActiveWorkSpace()) {
                ...
                $company_settings = getCompanyAllSetting();
                if (!empty($company_settings['Lead Emails']) && $company_settings['Lead Emails']  == true) {
...
$sanitizedWorkspace = Str::upper(Str::slug($workspaceName, '_'));
            ...
            foreach ($file_data as $key => $row) {
                ...
                if ($leads->isEmpty()) {
                    try {
                        ...
                        $lead = Lead::create([
                            'lead_id' => "{$sanitizedWorkspace}_{$uniqueId}",
    ...
    public function taskStore($id, Request $request)
    {
        ...
        if ($usr->isAbleTo('lead task create')) {
            ...
            if ($lead->created_by == $creatorId && $lead->workspace_id == $getActiveWorkSpace) {
                ...
                $company_settings = getCompanyAllSetting();
                if (!empty($company_settings['New Task']) && $company_settings['New Task']  == true) {
                    $tArr = [
                        'lead_name' => $lead->name,
                        'lead_pipeline' => $lead->pipeline->name,
                        'lead_stage' => $lead->stage->name,
    ...
    public function syncLeads(Request $request)
    {
        ...
        if (!is_array($rows) || empty($rows)) {
            return response()->json(['success' => false, 'message' => 'Invalid or empty data'], 400);
        }

        $count = 0;

        foreach ($rows as $row) {
            // Ensure we have a lead_id or id
            if (!isset($row['lead_id']) && !isset($row['id'])) continue;
        ...
        return response()->json([
            'success' => true,
            'message' => "$count records synced successfully",
        ]);
    }
    ...
    public function notifySlack(Request $request)
    {
        ...
        $response = Http::withHeaders([
            ...
            'channel' => '#general',  // or any channel/user your bot has access to
            'text' => '🚀 ERP successfully connected to Slack!',
        ]);
    ...
    public function sendMessage(Request $request)
    {
        ...
        if (!$response->json('ok')) {
            return back()->with('error', 'Slack error: ' . $response->json('error'));
        }

        return back()->with('success', 'Message sent to Slack successfully!');
    }
    ...
    public function receiveReply(Request $request)
    {
       $data = $request->all();
        ...
        // ✅ Step 1: Slack verification (respond to the challenge)
        if (isset($data['challenge'])) {
            return response($data['challenge'], 200)
                ->header('Content-Type', 'text/plain');
        }
    
        // ✅ Step 2: Handle actual Slack events (e.g. messages, reactions)
        if (isset($data['event'])) {
            $event = $data['event'];
    
            // Example: message posted in Slack
            if ($event['type'] === 'message' && empty($event['subtype'])) {
                SlackMessage::create([
    ...
    {
        $messages = SlackMessage::orderBy('created_at', 'asc')->get();
        return view('lead::slack.chat', compact('messages'));
    }
}
