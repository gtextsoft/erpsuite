            ...
            if ($get_amount > $invoice->getDue()) {
                ...
                if (isset($response['id']) && $response['id'] != null) {
                    ...
                    if ($request->type == 'invoice') {
                        ...
                        return redirect()->route('pay.salesinvoice',\Illuminate\Support\Facades\Crypt::encrypt($invoice_id))->with('error', $response['message'] ?? 'Something went wrong.');
                        ...
                        return redirect()->route('pay.fees', Crypt::encrypt($invoice_id))->with('error', $response['message'] ?? 'Something went wrong.');
                    }
                    elseif($type == 'childfeepayment') {
                        return redirect()->route('childfee.show', Crypt::encrypt($invoice_id))->with('error', $response['message'] ?? 'Something went wrong.');
                    }
                }
        ...
        if ($type == 'invoice') {
            ...
            if ($invoice) {
                ...
                try {
                    ...
                    $invoice->save();
                    $type = 'feereceipt';
                    event(new PaypalPaymentStatus($invoice, $type, $payments));
        ...
        elseif($type == 'childfeepayment'){
            ...
            if ($invoice) {
                ...
                try {
                    ...
                    return redirect()->route('childfee.show', \Illuminate\Support\Facades\Crypt::encrypt($invoice_id))->with('success', $e->getMessage());
                }
            } else {
                return redirect()->route('childfee.show', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('error', __('Retainer not found.'));
            }
        }
    }
