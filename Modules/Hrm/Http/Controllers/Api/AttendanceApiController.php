...
class AttendanceApiController extends Controller
{
        ...
        if($request->type == 'clockin'){
            ...
            try{
                ...
                $employeeId      = $request->user_id;
                 $activeWorkspace = $request->workspace_id ?? $authUser->active_workspace;
            if (!$activeWorkspace) {
                return response()->json(['status' => 0, 'message' => 'No active workspace found for the user.'], 403);
            }
	...
	public function checkClockStatus(Request $request, $user_id)
{
    $attendance = Attendance::where('employee_id', $user_id)
        ->where('date', date('Y-m-d'))
        ->where('clock_out', '00:00:00') // User has not clocked out
        ->first();

    if ($attendance) {
        return response()->json([
            'status' => 1,
            'message' => 'User is clocked in.',
