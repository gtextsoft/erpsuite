...
@section('page-action')
    <div class="d-flex">
        ...
        @permission('todo manage')
            ...
            <a class="btn btn-sm btn-primary me-2" data-bs-toggle="tooltip" href="{{ route('to-do.calendar') }}"
...
@endsection
@php
    $UsersData = \Modules\ToDo\Entities\ToDo::getUsersData();
@endphp

@section('content')
    @if (!empty($stages) && Laratrust::hasPermission('todo manage'))
        <div class="row">
            <div class="col-sm-12">
                                    ...
                                    @foreach ($stage->tasks as $task)
                                        <div class="card" id="{{ $task->id }}">
                                            <div class="card-header border-0 pb-0 position-relative">
                                                <div>
                                                    @if ($task->priority == 'Low')
                                                        <div class="badge bg-success p-2 px-3 me-2">
                                                            {{ $task->priority }}
                                                        </div>
                                                    @elseif($task->priority == 'Medium')
                                                        <div class="badge bg-warning p-2 px-3 me-2">
                                                            {{ $task->priority }}
                                                        </div>
                                                    @elseif($task->priority == 'High')
                                                        <div class="badge bg-danger p-2 px-3 me-2">
                                                            {{ $task->priority }}
                                                        </div>
                                                    @endif

                                                    <div class="badge bg-secondary p-2 px-3">
                                                        {{ !empty($task->sub_module) ? $task->sub_module : '----' }}
                                                    </div>
                                                </div>
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            @permission('todo complete')
                                                                <a class="dropdown-item" data-ajax-popup="true" data-size="md"
                                                                    data-title="{{ $task->status != $todoStage->id ? __('Mark Completed?') : __('Completed') }}"
                                                                    data-bs-original-title="{{ $task->status != $todoStage->id ? __('Mark Completed?') : __('Completed') }}"
                                                                    data-title="{{ __('View Task') }}"
                                                                    data-url="{{ route('to-do.status', $task->id) }}">

                                                                    @if ($task->status == $todoStage->id)
                                                                        <i class="ti ti-check"></i>
                                                                        {{ $task->status != $todoStage->id ? __('Mark Completed?') : __('Completed') }}
                                                                    @else
                                                                        <input type="checkbox" class="form-check-input d-none"
                                                                            disabled="disabled" name="status"
                                                                            id="{{ $task->id }}"
                                                                            {{ $task->status == $todoStage->id ? 'checked' : '' }}>
                                                            ...
                                                            @permission('task show')
                                                                <a class="dropdown-item" data-ajax-popup="true" data-size="lg"
                                                                    data-title="{{ __('View Task') }}"
                                                                    data-url="{{ route('to-do.show', $task->id) }}"><i
                                                                        class="ti ti-eye"></i> {{ __('View') }}</a>
                                                            @endpermission
                                                            @permission('task edit')
                                                                <a class="dropdown-item" data-ajax-popup="true" data-size="lg"
                                                                    data-title="{{ __('Edit Task') }}"data-url="{{ route('to-do.edit', $task->id) }}"><i
                                                                        class="ti ti-pencil"></i> {{ __('Edit') }}</a>
                                                            @endpermission
                                                            @permission('task delete')
                                                                <form id="delete-form-{{ $task->id }}"
                                                                    action="{{ route('to-do.destroy', $task->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <a href="#"
                                            ...
                                            <div class="card-body pt-0">
                                                <div class="row">
                                                    <div class="p-3">
                                                        <a href="#" data-url="{{ route('to-do.show', $task->id) }}"
                                                ...
                                                <div class="d-flex align-items-center justify-content-between pt-2 mt-2 border-secondary-subtle border-top">
                                                    <div class="user-group">
                                                        <img alt="image" data-bs-toggle="tooltip"
                                                            data-bs-placement="top" title="{{ $task->assignedByUser->name }}"
                                                            @if ($task->assignedByUser->avatar) src="{{ get_file($task->assignedByUser->avatar) }}"
                                                            @else src="{{ get_file('avatar.png') }}" @endif
                                                            class="rounded-circle " width="40px" height="40px">
                                                    </div>
                                                    <div class="user-group">
                                                        @if (count($UsersData) > 0)
                                                            @foreach (explode(',', $task->assigned_to) as $user)
                                                                <img alt="image" data-bs-toggle="tooltip"
                                                                    data-bs-placement="top"
                                                                    title="{{ $UsersData[$user]['name'] }}"
                                                                    @if ($UsersData[$user]['avatar']) src="{{ get_file($UsersData[$user]['avatar']) }}"
                                                                    @else
