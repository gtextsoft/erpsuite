...
class StripeController extends Controller
{
    ...
    public function invoicePayWithStripe(Request $request)
    {
        if ($request->type == "invoice") {
            ...
            $user_id = $invoice->created_by;
            $wokspace = $invoice->workspace;
        } elseif ($request->type == "feesreceipt") {
            $invoice = \Modules\LegalCaseManagement\Entities\FeesReceipt::find($request->invoice_id);
            $user_id = $invoice->created_by;
            $wokspace = $invoice->workspace;
        }elseif($request->type == 'childfeepayment'){
            $invoice = \Modules\ChildcareManagement\Entities\ChildFee::find($request->invoice_id);
            $user_id = $invoice->created_by;
            $wokspace = $invoice->workspace;
        }
        ...
        if (isset($this->is_stripe_enabled) && $this->is_stripe_enabled == 'on' && !empty($this->stripe_key) && !empty($this->stripe_secret)) {
            ...
            if ($request->type == "invoice") {
                ...
                $invoice_payID = $invoice->invoice_id;
                $invoiceID = $invoice->id;
                $printID = $invoice->bill_number;

            }elseif($request->type == 'childfeepayment'){
                $invoice = \Modules\ChildcareManagement\Entities\ChildFee::find($request->invoice_id);
                $invoiceID = $invoice->id;
                $printID = $invoice->fee_number;
            }
            ...
            if ($invoice) {
                ...
                if ($request->type == 'invoice') {
                    ...
                    return redirect()->route('pay.fees', Crypt::encrypt($invoiceID))->with('error', __('Fee Receipt is deleted.'));
                }elseif($request->type == 'childfeepayment'){
                    return redirect()->route('childfee.show', encrypt($invoiceID))->with('error', __('Child Fee is deleted.'));
                }
            }
        } else {
            ...
            if ($request->return_type == 'success') {
                if ($type == 'invoice') {
                    ...
                    if (!empty($invoice_id)) {
                        ...
                        try {
                            $stripe = new \Stripe\StripeClient(!empty(company_setting('stripe_secret', $invoice->created_by, $invoice->workspace)) ? company_setting('stripe_secret', $feeReceipt->created_by, $feeReceipt->workspace) : '');
                        ...
                        if ($invoice) {
                            try {
                                if ($request->return_type == 'success') {
                                    ...
                                    $payments->currency = isset($get_data['currency']) ? $get_data['currency'] : 'INR';
                                    $payments->method = __('STRIPE');
                                    $payments->reciept = $receipt_url;
                                    $payments->save();
                                    ...
                                    $invoice->save();
                                    $type = 'feereceipt';
                                    event(new StripePaymentStatus($invoice, $type, $payments));
                                    ...
                                    return redirect()->route('pay.fees', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('error', __('Transaction has been failed!'));
                                }
                            } catch (\Exception $e) {
                                return redirect()->route('pay.fees', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('error', __('Transaction has been failed!'));
                            }
                ...
                } elseif($type == 'childfeepayment'){
                    if (!empty($invoice_id)) {
                        $invoice_id = decrypt($invoice_id);
                        $invoice = \Modules\ChildcareManagement\Entities\ChildFee::find($invoice_id);
                        $session_data = $request->session()->get('comapany_stripe_data');
                        try {
                            $stripe = new \Stripe\StripeClient(!empty(company_setting('stripe_secret', $invoice->created_by, $invoice->workspace)) ? company_setting('stripe_secret', $invoice->created_by, $invoice->workspace) : '');
                            $paymentIntents = $stripe->paymentIntents->retrieve(
                        ...
                        if ($invoice) {
                            try {
                                if ($request->return_type == 'success') {
                                    $payments = new \Modules\ChildcareManagement\Entities\FeePayment();
                                    $payments->childfee_id = $invoice->id;
                                    $payments->amount = isset($get_data['amount']) ? $get_data['amount'] : 0;
                                    $payments->date = date('Y-m-d');
                                    $payments->order_id = $orderID;
                                    $payments->currency = isset($get_data['currency']) ? $get_data['currency'] : 'INR';
                                    ...
                                    $payments->save();
                                    $payment = \Modules\ChildcareManagement\Entities\FeePayment::where('childfee_id', $invoice->id)->sum('amount');
                                    ...
                                    event(new StripePaymentStatus($invoice, $type, $payments));
                                    return redirect()->route('childfee.show', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('success', __('Payment added Successfully'));
                                } else {
                                    return redirect()->route('childfee.show', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('error', __('Transaction has been failed!'));
                                }
                            } catch (\Exception $e) {
                                return redirect()->route('childfee.show', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('error', __('Transaction has been failed!'));
                            }
                        } else {
                            return redirect()->route('childfee.show', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with('error', __('Child Fee not found.'));
                        }
                    } else {
                        return redirect()->back()->with('error', __('Transaction has been failed.'));
                    }
